...
class Raid:
    def __init__(self, mapping, addr_desc, addr, disk_count, seek_speed, seed, balance, read_fraction, window, animate_delay):
        ...
        if self.mapping == 0:
            ...
            print(f'mapping, {self.mapping}, not supported')
            exit(1)
    ...
    def PrintStats(self):
        ...
        print (f'  Requests:   {self.logical_requests}')
        print (f'  Avg Latency: {(float(self.latency_total) / float(self.logical_requests))}')
        return
...
if options.print_options:
    print (f'OPTIONS seed {options.seed}')
    print (f'OPTIONS addr {options.addr}')
    print (f'OPTIONS addr_desc {options.addr_desc}')
    print (f'OPTIONS seek_speed {options.seek_speed}')
    print (f'OPTIONS window {options.window}')
    print (f'OPTIONS policy {options.policy}')
    print (f'OPTIONS compute {options.compute}')
    print (f'OPTIONS read_fraction {options.read_fraction}')
    print (f'OPTIONS graphics {options.graphics}')
    print (f'OPTIONS animate_delay {options.animate_delay}')
    print ('')

if options.window == 0:
    print (f'Scheduling window ({options.window}) must be positive or -1 (which means a full window)')
    sys.exit(1)
